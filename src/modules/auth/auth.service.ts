...
export class AuthService {
  ...
  async registerByWechat(code: string): Promise<any> {
    try {
      const res = await Axios.get<any, AxiosResponse<WechatRes>>(
      ...
      console.log(
        ' res.data = ' + JSON.stringify(res.data) + ' code = ' + code,
      );
      if (!res.data.errcode || res.data.errcode === 0) {
        let auth = await this.authThirdModel
          .findOne({ openid: res.data.openid })
          .exec();
        if (auth) {
